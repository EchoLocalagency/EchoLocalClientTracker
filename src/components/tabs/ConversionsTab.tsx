    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 32, fontWeight: 700 }}>{conversionRate}</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Phone clicks / sessions</div>
        </div>
      </div>
        ...
        <div style={{ height: 240 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={phoneData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="calls" fill="#00CED1" radius={[4, 4, 0, 0]} name="Phone Clicks" />
              {hasFormTracking && (
                <Bar dataKey="forms" fill="#FFD700" radius={[4, 4, 0, 0]} name="Form Submits" />
              )}
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              <th style={{ textAlign: 'left', padding: '8px 12px', color: 'var(--text-muted)', fontWeight: 500, fontSize: 12 }}>Period</th>
              <th style={{ textAlign: 'right', padding: '8px 12px', color: 'var(--text-muted)', fontWeight: 500, fontSize: 12 }}>Sessions</th>
              <th style={{ textAlign: 'right', padding: '8px 12px', color: 'var(--text-muted)', fontWeight: 500, fontSize: 12 }}>Phone Clicks</th>
              {hasFormTracking && <th style={{ textAlign: 'right', padding: '8px 12px', color: 'var(--text-muted)', fontWeight: 500, fontSize: 12 }}>Form Submits</th>}
              <th style={{ textAlign: 'right', padding: '8px 12px', color: 'var(--text-muted)', fontWeight: 500, fontSize: 12 }}>Conv. Rate</th>
            </tr>
          </thead>
          <tbody>
            {sorted.slice().reverse().map((r) => {
              ...
              return (
                <tr key={r.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: 'var(--text-muted)' }}>{r.ga4_sessions?.toLocaleString() ?? '--'}</td>
                  ...
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: 'var(--accent-teal)' }}>{rate}</td>
                </tr>
              );
            })}
          </tbody>
