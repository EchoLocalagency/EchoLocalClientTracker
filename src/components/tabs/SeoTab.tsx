...
export default function SeoTab({ reports, queries, latestReport }: SeoTabProps) {
  if (!latestReport) {
  ...
  const impressionsData = sorted.map((r) => ({
  ...
  const organicData = sorted.map((r) => ({
    date: new Date(r.run_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    organic: r.ga4_organic ?? 0,
    total: r.ga4_sessions ?? 0,
  }));

  const positionData = sorted.map((r) => ({
    date: new Date(r.run_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
        ...
        <div style={{ height: 220 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={impressionsData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="impressions" stroke="#00CED1" strokeWidth={2} dot={false} name="Impressions" />
              <Line type="monotone" dataKey="clicks" stroke="#FFD700" strokeWidth={2} dot={false} name="Clicks" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div style={{ height: 220 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={organicData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11 }} />
              <Tooltip content={<ChartTooltip />} />
              <Line type="monotone" dataKey="total" stroke="var(--text-muted)" strokeWidth={1.5} dot={false} name="Total" />
              <Line type="monotone" dataKey="organic" stroke="#00CED1" strokeWidth={2} dot={false} name="Organic" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div style={{ height: 180 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={positionData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Tooltip content={<ChartTooltip />} />
              <Line type="monotone" dataKey="position" stroke="#00CED1" strokeWidth={2} dot={false} name="Avg Position" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
          ...
          <tbody>
            {sortedQueries.map((q, i) => (
              <tr key={q.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '10px 12px', textAlign: 'right', color: 'var(--text-muted)' }}>{q.impressions.toLocaleString()}</td>
                <td style={{ padding: '10px 12px', textAlign: 'right', color: 'var(--text-muted)' }}>{q.clicks.toLocaleString()}</td>
